...
{// service that creates and manages HttpClient objects ^
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        HttpResponseMessage response = await client.GetAsync(
            $"users/{username}/events?per_page={page}&per_page={perPage}",
            cancellationToken);
    ...
    {
        HttpClient client = httpClientFactory.CreateClient("github");// uses the configured client from dep inj
