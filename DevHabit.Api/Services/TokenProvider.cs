...
public sealed class TokenProvider(IOptions<JwtAuthOptions> options)
{
    ...
    private string GenerateAccessToken(TokenRequest tokenRequest)
    {
        ...
        var securityKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_jwtAuthOptions.Key));// signing key
        var credentials = new SigningCredentials(securityKey, SecurityAlgorithms.HmacSha256);// key and algo to sign token

        List<Claim> claims =
        [
            new(JwtRegisteredClaimNames.Sub, tokenRequest.UserId),
            new(JwtRegisteredClaimNames.Email, tokenRequest.Email),
            ..tokenRequest.Roles.Select(role => new Claim(ClaimTypes.Role, role))// .. = claims.AddRange
        ];
