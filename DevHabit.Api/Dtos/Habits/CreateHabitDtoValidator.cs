...
public sealed class CreateHabitDtoValidator : AbstractValidator<CreateHabitDto>
{
    ...
    public CreateHabitDtoValidator()
    {
        RuleFor(x => x.Name)
            ...
            .MinimumLength(3)
            .MaximumLength(50)
            .WithMessage("Habit name must be between 3 and 50 characters.");
        ...
        RuleFor(x => x.Target.Unit)
            ...
            .WithMessage($"Unit must be one between: {string.Join(", ", AllowedUnits)}");

        RuleFor(x => x.EndDate)
            .Must(date => date is null || date.Value > DateOnly.FromDateTime(DateTime.UtcNow))
            .WithMessage("End date must be in the future.");

        When(x => x.MilestoneTarget is not null, () =>
        {
            RuleFor(x => x.MilestoneTarget)
            .GreaterThan(0)
